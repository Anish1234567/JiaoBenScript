...
public:
    _BadStmt<NodeType>(const NodeType &node, const std::string &msg)
        : CompileError(msg, node.pos_start, node.pos_end)
    {}
};
...
public:
    explicit BadReturn(const S_Return &node) : _BadStmt<S_Return>(node, "Bad return") {}
};


class BadBreak : public _BadStmt<S_Break> {
public:
    explicit BadBreak(const S_Break &node) : _BadStmt<S_Break>(node, "Bad break") {}
};


class BadContinue : public _BadStmt<S_Continue> {
public:
    explicit BadContinue(const S_Continue &node) : _BadStmt<S_Continue>(node, "Bad return") {}
};
